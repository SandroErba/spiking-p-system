...
def rules_train_SNPS(spike_train):
    ...
    # --- CORREZIONE CRITICA: RESHAPE A 4 DIMENSIONI ---
    rows_cols = int(Config.IMG_SHAPE / Config.BLOCK_SHAPE) # 7
    levels = Config.NUM_INPUT_LEVELS # 5
    channels = Config.NUM_CHANNELS # 3
    
    # Rimodella in (Canali, Livelli, Righe, Colonne) -> (3, 5, 7, 7)
    reshaped_counts = snps.layer_2_firing_counts.reshape((channels, levels, rows_cols, rows_cols))
