...
def launch_gray_SNPS():
    data = MedMnist.get_blood_mnist_data()
    train_red = data[0][0]
    ...
    num_images = getattr(Config, "TRAIN_SIZE", None) or snps.edge_output.shape[1]
...
def show_images(output_array, img_size=26, max_images=Config.TRAIN_SIZE):
...
def kernel_SNPS_csv():
    ...
    # Aggiorna Config
    Config.IMG_SHAPE = 28
    Config.KERNEL_SHAPE = KERNEL_SIZE
    Config.KERNEL_NUMBER = 1
    Config.SEGMENTED_SHAPE = OUTPUT_DIM
    Config.NEURONS_LAYER1 = LAYER1_SIZE
    Config.NEURONS_LAYER2 = LAYER2_SIZE
    Config.NEURONS_LAYER1_2 = LAYER1_SIZE + LAYER2_SIZE
    Config.NEURONS_TOTAL = Config.NEURONS_LAYER1_2 + LAYER2_SIZE
