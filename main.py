...
# msnp = MSNPSystem(configurationVector=c0, spikingVector=spikingVector, spikingTransitionMatrix=M, netGainVector=netGainVector, ruleVector=ruleVector, max_steps=50, deterministic=True,applyingRuleVector=np.array([0,1,2,3,4,5]), targetVector=np.array([1,1,0,-3,3,3]))
# msnp.execute(verbose=True)
